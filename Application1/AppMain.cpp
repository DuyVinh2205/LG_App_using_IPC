...
void AppMain::getDataFromJson()
{
    QFile json(":/other/employeemodel.json");
    if(json.open(QIODevice::ReadOnly)) {
        QJsonParseError parseError;
        m_employee = QJsonDocument::fromJson(json.readAll(), &parseError).object();
        m_arrEmployees = m_employee.value("data").toArray();
        json.close();
